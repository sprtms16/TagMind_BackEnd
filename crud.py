...
def get_diary(db: Session, diary_id: int):
    return db.query(models.Diary).options(joinedload(models.Diary.tags)).filter(models.Diary.id == diary_id).first()

def get_diaries(db: Session, user_id: int, skip: int = 0, limit: int = 100):
    return db.query(models.Diary).filter(models.Diary.user_id == user_id).order_by(models.Diary.created_at.desc()).offset(skip).limit(limit).all()

def get_diaries_by_date(db: Session, user_id: int, date_str: str, skip: int = 0, limit: int = 100):
    ...
    ).order_by(models.Diary.created_at.desc()).offset(skip).limit(limit).all()

def update_diary(db: Session, db_diary: models.Diary, diary_update: schemas.DiaryUpdate):
...
def create_tag(db: Session, tag: schemas.TagCreate):
    db_tag = models.Tag(name=tag.name, category=tag.category, is_default=tag.is_default, tag_pack_id=tag.tag_pack_id)
    db.add(db_tag)
    db.commit()
    db.refresh(db_tag)
    return db_tag
...
def get_user_tags(db: Session, user_id: int):
    ...
    purchased_tags = db.query(models.Tag).join(models.TagPack).join(models.UserTagPack).filter(
...
def get_tag_pack_by_product_id(db: Session, product_id: str):
    return db.query(models.TagPack).filter(models.TagPack.product_id == product_id).first()

def user_owns_tag_pack(db: Session, user_id: int, tag_pack_id: int):
    return db.query(models.UserTagPack).filter(
        and_(models.UserTagPack.user_id == user_id, models.UserTagPack.tag_pack_id == tag_pack_id)
    ).first() is not None
...
def search_diaries(db: Session, user_id: int, query: str):
    return db.query(models.Diary).join(models.Diary.tags).filter(
        models.Diary.user_id == user_id,
        (models.Diary.title.ilike(f"%{query}%") | models.Diary.content.ilike(f"%{query}%") | models.Tag.name.ilike(f"%{query}%"))
    ).distinct().all()
